...
const ThemeToggle = ({ size = 'default', showLabel = false, variant = 'button' }) => {
  ...
  const variantClasses = {
    button: `relative overflow-hidden rounded-lg border-2 
    ...
    minimal: `relative p-2 rounded-full 
              transition-colors transform duration-300 ease-in-out
              ${darkMode 
    ...
    card: `relative p-4 rounded-xl border 
           transition-colors transform duration-300 ease-in-out
           ${darkMode 
  ...
  if (variant === 'switch') {
    return (
      <div className="flex items-center space-x-3">
        ...
        >
          <span
            className={`inline-block h-4 w-4 transform rounded-full bg-white border border-gray-300 
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      >
        ...
        {/* Ripple effect (dynamic radius) */}
        {isAnimating && (
          <span
            className={`absolute inset-0 ${variant === 'card' ? 'rounded-xl' : 'rounded-lg'} 
                        bg-blue-500 opacity-20 animate-ping`}
          />
        )}
      </button>
    </div>
