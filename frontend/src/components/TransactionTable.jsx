          ...
          {(transactions || []).map((tx) => (
            ...
            >
              ...
              <td className="px-4 py-2 flex gap-2">
                ...
                <button
                  onClick={() => setDeleteModal({ show: true, tx })}
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-xs text-center border border-red-500">
            <h3 className="text-lg font-bold mb-4 text-red-300">{t.confirmDeleteTransaction}</h3>
            <div className="flex gap-4 justify-center">
              <button
                className="px-4 py-2 rounded-lg bg-red-500 text-white font-semibold hover:bg-red-600 transition"
                onClick={() => {
                  onDelete(deleteModal.tx);
              ...
              <button
                ...
                onClick={() => setDeleteModal({ show: false, tx: null })}
              >{t.cancel}</button>
            </div>
          </div>
        </div>
